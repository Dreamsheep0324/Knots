...
- 全新关系图谱功能：以"我"为中心可视化呈现你与所有联系人的关系网络,支持自由拖拽节点、双指缩放与平移画布,让人物关系一目了然
- 图谱力导向视图：节点按力学模拟自然分布,人物间关系以流光虚线连接,线上标注关系类型；选中节点或连线时底部弹出详情卡片
- 图谱星系视图：联系人围绕"我"按亲密度分层旋转,内圈家人、外圈新识,轨道半径与转速随亲密度递减,形成层次分明的星系结构
- 图谱事件节点：力导向视图中可选择显示共同事件节点,事件与相关人物自动连线,事件类型图标智能匹配（如含"餐"显示餐厅图标、含"见/聚/面"显示人群图标）
- 关系编辑模式：长按人物节点进入编辑模式可绘制关系连线,选择关系类型后保存；长按连线可删除关系
- 人物关系功能：人物详情与编辑界面新增"人物关系"区块,可记录该联系人与其他人的社交关系,关系对象可以是 App 内联系人也可以是外部人物（仅填姓名）,支持关系类型、自定义关系词与备注
- 自定义关系类型：内置家人、朋友、同事、同学、邻居、其他 6 个预设关系类型,可在自定义类型管理中增删改,与内置类型一同在图谱与人物详情中使用
- 人物详情关系展示：人物详情页展示该联系人的所有人物关系,点击 App 内联系人关系可跳转其详情,外部人物关系仅展示姓名
- 首页力导向图预览：在显示设置中开启"首页力导向图"后,首页轨道罗盘替换为力导向关系图谱,点击人物节点直接跳转详情,人物关系网络在首页即可一览
- 图谱节点视觉层次：高亲密度人物添加呼吸光晕,"我"节点金色脉冲扩散光环,选中节点旋转双色光环,关系边流动光粒子与脉搏波动画
- 图谱亲密度图例：星系视图底部展示五档亲密度色点与名称,便于辨识轨道层次
- 关系类型选择弹窗：编辑关系时弹出底部弹窗选择关系类型,支持自定义类型

### Changed

- 弹窗视觉全面升级：事件、人物、关系连线三类详情弹窗改为纯白卡片+左侧色条+圆角阴影设计,统一与首页卡片视觉风格
- 力导向布局优化：人物节点与事件节点自然混合分布,弹簧长度按亲密度递增,中心引力与阻尼调校使布局稳定协调
- 星系视图轨道参数：轨道半径按亲密度五档分层,转速遵循开普勒近似（内圈快外圈慢）,轨道圆圈改为虚线样式
- 礼物说明文案更新：移除已不存在的金额描述,礼物往来改为支持分类、场合、地点等详细信息

### Removed

- 彻底删除自动推断关系逻辑：原基于共同圈子和共同事件自动生成的关系推断功能移除,仅保留用户手动建立的关系,关系来源清晰可控,图谱不再出现无意义自动连线

## [1.5.3] - 2026-07-18

### Added

- 纪念日功能彻底移除农历：所有纪念日统一按公历计算与显示,不再保留任何农历相关逻辑与界面元素,消除农历算法长期存在的显示错误
- 订阅复活路径：已过期订阅若把下次扣费日期改到未来,会自动恢复为有效状态,无需重新创建
- 待办统一语义接口：待办事项、提醒、想法待办三类数据原各自独立定义"完成"语义,现统一为共享接口,便于未来聚合查看

### Changed

- 领域层深度优化：基于模块深度审查报告完成 4 批共 61 项问题修复,覆盖关键数据破坏 Bug、硬约束执行、架构重构、死代码清理、跨文件重复消除与测试守护
- 花费字段彻底清除：事件类型原保留"借出/借入"两选项,事件与礼物原保留金额输入框,违反"全应用去除花费字段"约定,本次从领域层根源彻底清除,同步清理数据层与界面层全部相关代码
- 亲密度颜色统一来源：亲密度等级颜色原在领域层与界面层三处重复定义,导致首页与其他模块颜色不一致,现统一由领域层枚举派生,界面层不再持有副本
- 自定义类型删除不再误删其他字段：删除某类别（如爱好）的自定义类型时,原会从爱好/习惯/饮食/技能全部四个字段中误删同名值,现仅清理对应类别字段
- 联系人清空生日同步清理旧纪念日：编辑联系人把生日改为空时,原残留旧的生日纪念日导致详情页仍显示"距离生日 X 天",现同步删除
- 跨聚合写入事务保护：新建联系人生成生日纪念日、新建事件添加参与者、批量清理自定义类型等跨聚合写入原无事务,中途失败会留下半残数据,现改为事务化批量写入
- 聚合用例职责收敛：联系人详情聚合用例原混入想法写操作与收藏写能力,违反"聚合用例只读"原则,现收敛为纯读聚合,写操作下沉到专用写用例
- 备份仓库接口拆分：备份仓库原单一接口承担 5 类职责共 19 个方法,违反接口隔离原则,现拆分为备份、目录管理、图片卫生、指纹计算 4 个子接口,调用方按需依赖
- 联系人关系等级字段删除：联系人原同时有"关系等级"与"亲密度分数"两个语义重叠字段,关系等级经全仓搜索无界面引用,现删除该死字段,数据库相应迁移
- 首页统计类型安全强化：首页统计原用 14 路可变参数合并,位置错位会静默归零且无编译错误,现改为分组类型安全合并,类型不匹配直接编译失败
- 订阅默认提醒天数集中：订阅提醒天数原硬编码数字,现提取为常量,与领域层其他默认值集中管理
- 纪念日分类返回值语义化：纪念日按"全部/即将/已过"分类原返回无语义的三元组,易解构错位,现改为命名的数据类
- 足迹年份筛选稳定化：足迹按年筛选原为每条记录创建日历实例,现改用时间 API 一次性解析,且固定北京时区避免跨时区漂移
- 加密降级状态架构下沉：加密降级标记原为领域层全局可变单例,被数据层写入、界面层读取形成三向耦合,且无法重置,现迁移为依赖注入的可观测状态
- 联系人仓库接口分文件：联系人仓库原一个文件含三个独立接口,其他仓库均一文件一接口,现拆分为三个独立文件保持一致
- 星座查询简化：星座按月日查询原三分支含两段完全相同的分支体与不可达的兜底,现合并为单一查询表达式
- 日期计算模板抽取：日期计算工具原 5 个函数重复"提取月日、安全构造日期、滚到次年"模板,现抽取公共辅助函数,5 处复用
- 相对时间显示重构：相对时间显示原嵌套条件含多个魔法数字与重复判断,现重构为"过去/未来"两大分支并复用时间常量
- 周年纪念下次发生日期抽取：周年纪念计算（生日/重复事件）原 5 处复制粘贴同一模板,现抽取公共函数统一实现

### Fixed

- 修复删除某类别（如爱好）的自定义类型时,会从习惯/饮食/技能等其他字段中误删同名值,导致技能等数据被破坏且不可逆
- 修复首页统计测试 mock 了错误的方法名,3 个核心统计用例实际全部失败但被忽略,统计逻辑变更无任何回归保护
- 修复事件类型仍保留"借出/借入",事件与礼物仍保留金额输入框,违反全应用去除花费字段约定
- 修复亲密度等级颜色在领域层与界面层三处重复定义,导致首页朋友档颜色与其他模块不一致
- 修复无关联联系人的纪念日保存时崩溃：领域层默认值与数据层映射不一致导致外键约束违反
- 修复编辑联系人清空生日后旧的生日纪念日仍残留,详情页继续显示"距离生日 X 天"
- 修复跨聚合写入（新建联系人+生日纪念日、新建事件+参与者、批量清理自定义类型）无事务保护,中途失败留下半残数据
- 修复已过期订阅把下次扣费日期改到未来后仍显示为过期,无法自动复活
- 修复订阅"即将到期"统计把已过期订阅也算入,显示误导
- 修复星座按生日查询用设备时区解析,星座交界日出生的用户跨时区会显示错误星座
- 修复相册照片收藏键哈希算法对长字符串溢出,可能产生碰撞导致收藏状态串
- 修复纪念日分类返回三元组无语义,重构时极易解构错位

### Removed

- 清理纪念日农历功能：彻底删除农历相关字段、工具类、测试与界面逻辑,纪念日统一按公历
- 清理领域层死代码：calculateBirthdayInfo/calculateDaysUntil 等无生产调用方的工具方法、AppStrings.EventType 与枚举 displayName 重复的常量、Color.kt 中 5 个无引用的亲密度颜色常量、searchEventsByType 冗余查询变体
- 清理联系人死亡字段：relationshipLevel 经全仓搜索无界面与用例引用,从模型与数据库中彻底删除
