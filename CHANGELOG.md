...
- 界面基础层测试守护补全：为密度计算、事件图标解析、颜色解析、纪念日图标查表、天气图标查表、情绪图标查表、礼物类型样式等关键逻辑补充单元测试,从"几乎零覆盖"提升至关键纯函数全覆盖
- 头像裁剪算法纯函数化：裁剪变换矩阵从界面代码中剥离为独立纯函数,新增单元测试验证裁剪准确性
- 事件图标关键词智能匹配测试：覆盖餐食/见面/旅行/通话/礼物/聊天六大语义分支,确保自定义事件类型图标解析行为一致

### Changed

- 动画生命周期守卫补全：轨道日历旋转、连续旋转、全息卡片浮动/扫描线/微光等无限循环动画原在应用切后台时仍挂起协程占用资源,统一接入生命周期守卫,切后台即暂停、回前台平滑续播
- 动画协程重建修复：闪烁动画原每帧取消并重启协程,改为派生状态计算,消除每秒数十次协程创建销毁
- 禁用动画的资源浪费消除：全息卡片在动画被禁用时仍创建动画协程,改为按需创建,避免无意义计算
- 头像裁剪对话框拆分：单函数 180 行拆分为裁剪画布、按钮区、标题区、纯函数裁剪矩阵四个独立单元,布局调整不再影响手势处理
- 标签选择器参数精简：参数列表从 14 个精简为配置对象 + 回调对象 + 数据三组,新增功能不再膨胀参数
- 标签选择器函数体拆分：147 行主函数拆分为标签列表、对话框管理两个子组件,职责清晰
- 联系人选择器参数精简：业务参数从 9 个精简为配置对象 + 回调对象
- 日期选择器参数精简：业务参数从 8 个精简为配置对象 + 回调对象
- 全息卡片叠加层拆分：110 行叠加层函数拆分为扫描线、微光、四角标记三个独立绘制单元
- 确认对话框拆分：100 行函数拆分为标题区、内容区、按钮区三个子组件
- 全息卡片内容拆分：80 行函数提取动画偏移量数据类,主函数仅消费偏移量渲染
- 照片组件删除按钮统一：缩略图与拍立得两种照片槽位的删除按钮提取为共用组件,样式调整只需改一处
- 颜色体系统一：浅色背景色与语义背景色两套重叠定义合并为单一语义色体系,消除维护不一致
- 语义文字色深色模式适配：文字色原为明暗通用顶层常量,深色模式下与背景色适配方式不一致,改为按主题适配
- 错误色深色模式适配：错误色原被界面直接引用未走主题,深色模式下显示过亮,统一改用主题错误色
- 图标管理模块化：单文件承载纪念日/天气/情绪三套图标系统拆分为四份领域文件,并抽象通用图标注册表消除重复模式
- 图标查表重复键清理：通用图标表中两个重复键被合并
- 十六进制颜色解析统一：模块内两套颜色解析逻辑合并为单一可测试纯函数
- 硬编码尺寸迁移至统一常量：区块标题、圆角形状、对话框形状、信号进度条等多处散落尺寸值统一收纳到尺寸常量文件
- 区块标题图标背景尺寸常量化：徽章背景与图标尺寸新增专用常量,发丝级分隔线粗细统一为常量
- 信号进度条计算式命名化：信号格条的基础高度、步进值、宽度从字面量提取为命名常量
- 头像裁剪魔法数字命名化：缩放范围、输出尺寸、压缩质量、遮罩透明度、圆环透明度、描边宽度等全部命名化并添加注释
- 密度计算魔法数字命名化：密度计算中的系数与边界值全部命名化
- 全息卡片动画参数命名化：动画时长、偏移量、透明度等全部命名化
- 进度条魔法数字命名化：进度计算中的系数与边界值全部命名化
- 系统字体放大上限放宽：原上限 1.3 倍限制视障用户使用,放宽至 2.0 倍改善无障碍
- 拍立得照片旋转简化：旋转角度表达式简化并绑定照片地址,滚动回收不再抖动
- 头像组件尺寸类型升级：尺寸参数从整数改为尺寸类型,与界面体系一致
- 动画原语语义化封装：呼吸类动画新增语义化封装函数,调用方无需关心重复模式等细节
- 关系徽章样式封装：文字渲染细节参数提取为样式数据类,调用方无需关注渲染细节

### Fixed

- 修复信号进度条在最大值为零时除零产生异常值,导致绘制行为错误
- 修复拍立得照片滚动回收后旋转角度变化造成视觉抖动
- 修复搜索栏清除按钮、照片删除按钮、标签删除图标、分段切换按钮等多处点击目标低于最小标准,误触率高
- 修复联系人选择对话框在联系人数量多时撑满甚至超出屏幕
- 修复自定义事件类型"OTHER"含"话"字（如"对话"）被误判为通话类型,改为精确匹配"电话/致电/通话"
- 修复菜单与消息图标使用已弃用方向,迁移至自动镜像方向版本

### Removed

- 清理死文件：选择状态工具文件（全项目无引用）、想法类型样式文件（想法模块未使用）
- 清理死常量：信号青色、10 个深色标签配色、深色主色变体、深色文字色、边框色、胶带塑料色、事件花费青色等无引用颜色
- 清理过期别名：6 个已弃用的浅色背景色别名（已被语义背景色取代）
- 清理动画死代码：暂停动画引擎中无效的停止分支
- 清理动画令牌冗余：两个完全相同的缓动曲线合并为单一标准缓动
- 清理礼物类型硬编码颜色：内联颜色值改为引用已有语义色
- 重新生成静态检查基线：清理过期豁免条目,暴露真实待修复问题

## [1.6.1] - 2026-07-23

### Added

- 领域层测试守护补全：为联系人列表管理、联系人创建、菜谱标签管理、联系人表单参考数据等核心业务逻辑补充单元测试,关键写操作从"零保护"提升至完整覆盖
- 云同步解析器纯逻辑测试：WebDAV 协议的 PROPFIND 响应解析从前端网络代码中剥离为独立组件,新增 11 个边界场景测试覆盖百分号编码、字面加号、非法 XML 等
- 文件差异计算器单测：云端同步的文件比对逻辑补充 15 个测试用例,覆盖新增/修改/删除/无变更等场景
- 数据库迁移测试补全：为近期四个版本升级路径补充自动化验证,确保升级过程数据完整

### Changed

- 领域层与时区对齐：日期计算与连续打卡统计原使用设备默认时区,跨时区设备会出现日期错位与统计归零,统一对齐到北京时区
- 纪念日倒计时语义修正：纪念日"距离 X 天"原计算逻辑自相矛盾（已过日期显示正数、未来日期显示负数）,重新对齐为自然的"未来为正、已过为负"
- 足迹与相册按联系人筛选修复：多参与者事件原仅取首位参与者,筛选其他参与者时数据丢失,改为收集全部参与者,任一匹配即显示
- 联系人富模型增强：联系人与事件原为纯数据容器（贫血模型）,现补充亲密度等级、星座、是否对话、代表参与者等派生属性,业务规则内聚到模型本身,消除分散的重复判断
- 设置存储接口拆分：设置仓库原单一接口承担 5 类无关偏好（主题、AI、备份、UI、用户资料）共 26 个方法,违反接口隔离原则,拆分为 5 个子接口,调用方按需依赖
- 事件统计接口拆分：事件仓库原胖接口混合 4 类职责,新增独立的事件统计接口,首页统计改依赖精简接口
- 菜谱标签管理独立：菜谱仓库原混入标签管理职责,抽出独立的标签仓库,接口职责更清晰
- UseCase 入口统一：15 个用例的入口方法原命名不一致（invoke/run/execute/get）,统一为 operator fun invoke,调用方语法一致
- 响应式方法命名统一：观察型方法原 Once/Sync 后缀混乱,统一为 observeXxx 命名风格
- 首页设置下沉：首页轨道/力导向图模式切换原界面层直接读配置,下沉到用例层,与其他设置访问方式一致
- 云同步上传顺序纠正：原"先删旧数据库→后写清单",中途失败清单指向已删文件导致云端备份永久不可恢复；改为"先写清单→后删旧数据库",旧库多留存片刻无代价
- 云同步下载隔离区模式：原下载流程在数据库恢复成功前就删除本地图片,恢复失败时旧库引用的图片已被物理删除无法找回；改为待删图片先移入隔离区,恢复成功后清理,失败时移回原位
- 新设备覆盖防护：新设备首次安装时本地空数据库的修改时间晚于云端清单时间,原逻辑误判为"本地有更新"允许上传,导致空库覆盖云端完整备份；现检测到云端已有备份而本设备从未同步过时,拒绝上传并引导先下载恢复
- 恢复快照清理：数据库恢复成功后原遗留整库明文副本（.before_restore）永不清理,每次恢复都积累一份；现成功路径显式删除快照,并在启动时兜底清理旧版本遗留
- 回滚结果语义化：数据库回滚原无快照时直接返回成功,实际半写入的损坏库留在原地；现返回枚举化的回滚结果（已恢复/已清理/失败）,上层可给出准确文案
- 回滚范围纳入设置：恢复回滚原仅恢复数据库,设置文件已被新备份覆盖则与旧库混跑两代；现快照范围纳入设置文件,一并回滚
- 旧版备份恢复路径修复：旧版全量备份列表从根目录列出,下载却去子目录取同名文件,两个路径必有一个是错的；现回退下载改用根目录路径,与列表一致
- 同步互斥与状态守卫：同步流程原无锁,双击或上传中点下载会触发两条流水线竞态,后写者覆盖先写者的清单；现仓库层加锁串行化,界面层辅以状态守卫
- 插入冲突防御统一：四个聚合（联系人/联系人+纪念日/事件+参与者/圈子+成员）原对插入返回的 -1 主键不防御,后续插关联数据触发外键崩溃；统一抽取守卫,冲突即抛出并回滚事务
- 清空数据彻底化："清空所有数据"原仅清除数据库与图片,四个 SharedPreferences（含 AI 密钥与云盘密码）纹丝不动,二手设备场景存在隐私泄漏；现同步清除全部敏感配置
- 远端文件名消毒：云同步清单中的文件名原直接来自服务器可控,恶意服务器可构造 `../` 路径越界读写应用私有目录；现统一在下载、本地查询、本地删除三处入口校验
- 子目录创建异常对齐：远程子目录创建原吞掉所有异常,创建失败时后续每个文件都报莫名其妙的错误,根因被掩盖；现与根目录创建对齐,405 放行其余上抛
- 网络异常不再伪装空数据：列文件与下载小文件原把网络故障 catch 为"无数据",用户误以为云端为空；现网络异常上抛,404 才视为文件不存在
- 取消异常透传：网络请求原 catch 所有异常包括取消信号,导致协程取消语义被破坏；现取消异常显式重抛
- 恢复失败回滚已创建文件：SAF 备份恢复原失败时不清理已创建的损坏文件,残留文件会被当作有效备份列出；现失败时删除已创建文件
- AI 流式背压修复：AI 对话流式响应原用默认缓冲,高频 token 触发背压异常导致对话中断；改为无限制缓冲
- URL 编码语义修正：云同步 URL 原手动拼字符串编码,可能双重编码或漏编码；改用标准 URL 构建器自动处理 path segment 编码
- AI 地址统一处理：AI 对话地址原未去除尾部斜杠,测试连接通过但正式对话请求双斜杠路径返 404；现统一去除尾部斜杠
- 跨设备首次同步重传消除：下载恢复后本地文件修改时间变为下载时刻,与清单时间不符导致全部图片被判定需重传；现下载后对齐清单时间戳,仅真正变更的文件才传输
- 启动自动同步消费：原"启动时自动同步"开关开启后无任何效果,功能承诺与实现不符；现启动时按开关触发一次同步（已含新设备覆盖防护）
- 同步孪生代码消除：云同步客户端与仓库原各含四组逐行复制的下载/上传/清理循环,任一改动需同步改两处；统一抽取参数化方法,八组复制循环消除
- 更新检查客户端修正：应用更新检查原误用 AI 专用网络客户端（超长超时且无重试）,恰恰是最该享受重试的 GET 场景却没挂上；改用通用客户端
- 备份图片质量参数移除：备份配置原透传一个"图片质量"参数到全链路但从未被消费,属半成品特性；整链移除避免误导
- 注释与命名订正：多处历史注释与实现不符（迁移说明、网络重试说明、人物关系互斥约束）,逐处订正；12 个迁移文件命名风格统一
- 数据库迁移测试守护：迁移链 v46→v50 补完整测试,人物关系建表、清理预设类型、清理历史记录、删除冗余列等关键变更均有自动化验证

### Fixed

- 修复日期计算使用设备时区,跨时区设备纪念日倒计时显示错误
- 修复想法游戏化连续打卡统计使用设备时区,跨时区设备打卡中断或归零
- 修复纪念日"距离 X 天"语义自相矛盾,已过与未来方向反了
- 修复足迹按联系人筛选仅取首位参与者,多参与者事件筛选其他参与者时数据丢失
- 修复相册按联系人筛选同样的首参与者问题,且 URI 裁剪不一致
- 修复亲密度等级阈值与枚举声明脱钩,未来调整阈值会引入隐性 Bug
- 修复新建联系人首次设置生日时不创建生日纪念日,详情页缺失生日倒计时
- 修复菜谱份量缩放在非英语/非中文语言区域下格式化产生逗号小数点,导致解析失败
- 修复云同步上传流程先删旧数据库后写清单,中途失败云端备份永久不可恢复
- 修复云同步下载流程在数据库恢复前删除本地图片,恢复失败不可回滚
- 修复新设备空数据库可一键覆盖云端完整备份
- 修复数据库恢复成功后整库明文副本永久残留,每次恢复都积累一份
- 修复数据库回滚无快照时误报"已回滚",实际半写损坏库留在原地
- 修复恢复回滚不含设置文件,失败时旧库与新设置混跑两代
- 修复旧版全量备份恢复路径错误,列出在根目录下载去子目录必 404
- 修复同步流程可重入,双击或上传中点下载导致清单竞态
- 修复插入主键冲突返回 -1 未防御,后续插关联数据崩溃
- 修复"清空所有数据"未清除 AI 密钥与云盘密码等敏感配置
- 修复远端文件名未消毒,恶意服务器可越界读写应用私有目录
- 修复远程子目录创建失败被吞,后续每个文件报错根因被掩盖
- 修复网络故障被伪装成"无数据",用户误以为云端为空
- 修复取消信号被 catch 吞掉,协程取消语义被破坏
- 修复 SAF 备份恢复失败时残留损坏文件被当作有效备份列出
- 修复 AI 流式响应背压异常导致对话中断
- 修复云同步 URL 双重编码或漏编码
- 修复 AI 对话地址带尾部斜杠时测试通过但正式对话 404
- 修复跨设备首次同步后全部图片被误判为需重传
- 修复"启动时自动同步"开关开启后无任何效果
- 修复应用更新检查误用 AI 专用客户端,网络抖动时不重试

### Removed

- 清理领域层死代码：BackupManifest/ManifestEntry 死类、Todoable 通用接口（无业务价值）、ContactRelation.source 冗余字段、25 处死 import 与通配符 import
- 清理数据层死代码：15 个零引用的 DAO 方法、ChatResponse/ChatChoice 死类（流式解析不使用）、空测试目录、BackupImageQuality 半成品参数（透传全链路但从未消费）
- 清理测试反向锁定 Bug：1.6.0 之前的部分测试反向锁定 buggy 行为为"正确",修复实现 Bug 后必须同步改测试
