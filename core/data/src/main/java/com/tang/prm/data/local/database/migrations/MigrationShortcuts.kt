...
/**
 * 合并迁移快捷路径：v1 → v24，合并了原始 MIGRATION_1_10、MIGRATION_10_20、MIGRATION_20_24 的全部操作。
 *
 * 优化点：
 * - gifts 表直接以 v24 最终 schema（含外键）创建，跳过 v10 无外键版本的中间重建
 * - event_participants 表直接以 v24 最终 schema（含外键）重建，跳过中间步骤
 * - thoughts 表直接以 v24 最终 schema 创建，包含所有列
 * - circles 表直接以 v24 最终 schema 创建，包含所有列
 * - contacts 表一次性添加所有新列
 * - custom_types 表一次性添加所有新列
 */
val MIGRATION_1_24 = object : Migration(1, 24) {
    override fun migrate(db: SupportSQLiteDatabase) {
        // ===== 来自 MIGRATION_1_10 (v1→v10) =====

        // custom_types: 添加 icon 列
        db.execSQL("ALTER TABLE custom_types ADD COLUMN icon TEXT")
        // anniversaries: 添加 icon 列
        db.execSQL("ALTER TABLE anniversaries ADD COLUMN icon TEXT")

        // events: 重建表（移除旧列，保留兼容列）
        ...
        // ===== 来自 MIGRATION_10_20 (v10→v20) =====

        // gifts: 直接以 v24 最终 schema 创建（含外键），跳过 v10 无外键版本
        ...
        // thoughts: 直接以 v24 最终 schema 创建（含所有列），跳过中间 ALTER TABLE
        ...
        // contacts: 添加 knowingDate 列
        db.execSQL("ALTER TABLE contacts ADD COLUMN knowingDate INTEGER")

        // circles: 直接以 v24 最终 schema 创建（含所有列），跳过中间 ALTER TABLE
        ...
        // favorites: 清理照片收藏标题中的📷前缀
        db.execSQL("UPDATE favorites SET title = REPLACE(title, CHAR(55357) || CHAR(56825) || ' ', '') WHERE sourceType = 'PHOTO'")

        // ===== 来自 MIGRATION_20_24 (v20→v24) =====

        // event_participants: 清理孤立数据后直接以 v24 最终 schema 重建（含外键）
