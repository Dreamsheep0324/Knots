...
val SemanticCoralText: Color
    @Composable @ReadOnlyComposable get() = if (isSystemInDarkTheme()) Color(0xFFF87171) else Color(0xFFDC2626)
val SemanticGreenBg: Color
    @Composable @ReadOnlyComposable get() = if (isSystemInDarkTheme()) Color(0xFF1A3A2E) else Color(0xFFD1FAE5)
val SemanticIndigoBg: Color
...
/** 解析十六进制颜色字符串（支持 #RRGGBB / #AARRGGBB / 无前缀），失败返回 null。 */
internal fun parseHexColorOrNull(hex: String): Color? = runCatching {
...
fun String?.toComposeColor(fallback: Color): Color {
    ...
    return parseHexColorOrNull(this) ?: fallback
}
